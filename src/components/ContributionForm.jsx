...
export default function ContributionForm() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 md:px-8 py-16 font-sans">
      <div className="glass rounded-3xl p-8 md:p-12 border border-[rgba(212,175,55,0.15)] shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-[var(--accent-color)] opacity-[0.02] rounded-bl-full pointer-events-none"></div>
        
        <div className="text-center mb-10">
          <span className="text-[10px] uppercase tracking-[0.2em] text-[var(--accent-color)] font-bold block mb-2">{t('registry_contributions')}</span>
          <h2 className="text-3xl md:text-4xl font-serif font-bold text-gradient-gold mb-3">{t('contribute_title')}</h2>
          <p className="text-xs text-[var(--text-secondary)] max-w-xl mx-auto leading-relaxed">{t('contribute_desc')}</p>
        </div>

        {status === 'success' ? (
          <div className="text-center py-12 bg-emerald-500/5 rounded-2xl border border-emerald-500/20 animate-fade-in">
            <span className="text-4xl mb-4 block">🕉️</span>
            <h3 className="text-2xl font-serif text-emerald-500 mb-2">Contribution Submitted Successfully</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {validationError && (
              <div className="p-4 bg-red-500/10 border border-red-500/20 text-red-500 rounded-xl text-xs flex items-center gap-2 animate-shake">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input 
                  ...
                  required
                  className="w-full px-4 py-2.5 rounded-lg bg-[var(--bg-card)] border border-[var(--border-color)] text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-color)] transition-all font-sans text-sm outline-none"
            ...
            <div>
              ...
              <textarea 
                ...
                required
                rows={5}
                className="w-full px-4 py-2.5 rounded-lg bg-[var(--bg-card)] border border-[var(--border-color)] text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-color)] transition-all font-sans text-sm outline-none resize-y"
                placeholder="Share the history, rituals, lineage details, or specific files associated with the deity..."
              />
            </div>

            {/* Media Upload Component */}
            <div className="border border-dashed border-[var(--border-color)] rounded-xl p-6 bg-[var(--bg-secondary)]/50 text-center">
              <span className="block text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider mb-2">{t('upload_visuals')}</span>
              <p className="text-[10px] text-[var(--text-muted)] mb-4 font-sans">Support JPG, PNG, WEBP images representing the temple landscape.</p>
              
              <label className="inline-flex items-center gap-2 px-5 py-2.5 bg-[var(--bg-card)] hover:bg-[var(--bg-secondary)] text-xs border border-[var(--border-color)] hover:border-[var(--accent-color)] rounded-lg text-[var(--text-primary)] font-semibold cursor-pointer transition-all shadow-sm">
                {uploading ? (
                  <>
              ...
              {attachments.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-2 justify-center">
                  {attachments.map((url, i) => (
                    <div key={i} className="relative w-14 h-14 border border-[var(--border-color)] rounded overflow-hidden group">
                      <img src={url} alt="Attachment" className="w-full h-full object-cover" />
                      <button 
                        type="button" 
                        onClick={() => removeAttachment(i)}
                        className="absolute inset-0 bg-red-950/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity text-white text-xs"
                      >
            ...
            <div className="flex justify-end">
              <button 
                ...
                className="flex items-center gap-2 px-8 py-3 bg-[var(--accent-color)] text-white font-semibold rounded-lg hover:brightness-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed text-sm cursor-pointer"
              >
                {status === 'submitting' ? 'Submitting...' : t('submit_entry')}
                <Send className="w-4 h-4" />
              </button>
            </div>
            {status === 'error' && (
              <p className="text-red-500 text-xs mt-2 text-right">An error occurred during submission. Please check inputs and try again.</p>
            )}
          </form>
        )}
      </div>
